<template>
  <div>
    ...
    <div class="tableau" @keyup.enter="colorTarget(i, 1)">
        ...
        <transition-group type="transition" :name="'flip-list'">
          <li v-for="element in list" :key="element.order"> 
            <ul class="interne">
              ...
              <li v-for="(message, i) in list2" :key="message.id" @click="colorTarget(i, 0, $event)">{{message[element.name]}}</li>
              <li>{{element.order}}</li>
            </ul>
          </li> 
        </transition-group>
  ...
  methods:{
    ...
    colorTarget (key, increment, e) {
      ...
      for (let i = 0; i < ul.length; i++) {
        ...
        li[key+1].style.backgroundColor = "#556677";
        li[key+1].style.color = "#ffffff";
      }
